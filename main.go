...
func main() {
	...
	// Chaek for hash
	if datafile.GetHash(filename) != standardHash || datafile.GetHash(filename) != ShadowHash || datafile.GetHash(filename) != ThinkertoyHash {
		asciiLines, err := datafile.GetStrings(filename)
		if err != nil {
